defmodule PokerPlan.Rounds.Round do
  ...
  def handle_cast(:reload_round, %{round: round} = round_info) do
    ...
    IO.inspect(round, label: "reloading round ...")
  ...
  def handle_cast({:set_current_task, %PokerPlan.Data.Task{} = task}, round_info) do
    case round_info.current_task do
      nil -> nil
      previous_task_pid -> PokerPlan.Rounds.Task.stop(previous_task_pid)
    end

    {:ok, task_pid} = PokerPlan.Rounds.Task.start_link(task, round_info.users)
    PokerPlan.Rounds.Task.start(task_pid)
    task_info = PokerPlan.Rounds.Task.get(task_pid)
  ...
  def handle_cast(:clear_current_task, round_info) do
    IO.inspect("clearing current task ...")
